...
import com.example.diploma_mk.databinding.ActivityPlayBinding
import java.util.*
...
class PlayActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //проверяется какая карточка была выбрана и на основание этого берет нужные данные для тестирования
        if (baf == 1) {
            questions.clear()
            answer.clear()
            options.clear()
...
//            options = options3
//        }
            //копируй то что выше для большего количества
        initViews()
    ...
    private fun timeOverAlertDialog() {
        val builder = AlertDialog.Builder(this@PlayActivity)
        val view = LayoutInflater.from(this@PlayActivity).inflate(R.layout.time_ower, null)
        builder.setView(view)
